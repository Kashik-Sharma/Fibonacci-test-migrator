...
logger.info("Would you like to see a graph of the first %d Fibonacci numbers?", FIRSTPOINTS)
...
##############################
logger.info("Our first thought would be to fit an exponential to this data.")
logger.info("However, because of the large numbers involved, it's better to fit")
logger.info("a line to the log of the Fibonacci numbers.")
...
logger.info("
Fitting a line to the data, the best fit has slope %s and y-intercept %s", popt[0], popt[1])

# For large n, the slope approaches phi = (1+sqrt(5))/2.  Let's see what we got.
logger.info("We can compare this to the theoretical limit (applicable for large n),
  which should yield the golden ratio.")
logger.info("Compare the fit's prediction:  %s", math.exp(popt[0]))
logger.info("     to the golden ratio phi:  %s", fb.phi)
logger.info("
So after using only %d Fibonacci numbers, the fit behavior differs from
     the theoretical limit by %s%%", FIRSTPOINTS, 100*(1- math.exp(popt[0])/fb.phi))

logger.info("
Would you like to see a graph of the line we fit to the first %d Fibonacci numbers, on a log scale?", FIRSTPOINTS)
...
if showit:
    ...
    pylab.show(block=False)
logger.info("From Figure 2, we can see that the fit seems to match the first %d Fibonacci numbers well,", int(FIRSTPOINTS))
logger.info("and extrapolating the fit to predict the %d Fibonacci number seems pretty good also.", int(PREDICTPOINT))
logger.info("
In fact, the fit predicts the %4d-th Fibonacci number to be %12E,", int(PREDICTPOINT), math.exp(fitlog500prediction))
logger.info("  within [%12E, %12E]. 
The actual value, %12E, is within these bounds.", lower500bound, upper500bound, actual500num)
...
logger.info("We have calculated the fit's prediction of the first %d Fibonacci numbers,
to compare to the actual Fibonacci numbers.", FIRSTPOINTS)